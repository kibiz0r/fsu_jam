			...
			if(container != null){
				...
				if(nc > 1){
					...
					for(var c:int = 0; c < container.numChildren; c++){
						...
						sortArray.push(new SimpleZSortVO(displayObject, transformedMatrix.position.z,transformedMatrix.position.y));
					}
					
					//Sort the array (Array.sort is still king of speed).
					//sortArray.sortOn(["screenY","screenZ"], [Array.NUMERIC, Array.NUMERIC | Array.DESCENDING]);
